...
// 功能处理源数据
void convertToOneDimensionalArray(const int* online, int (*fiveg_signal_strength)[Node/4]) {
    //中间数组[16*16]
    ...
    for(int i = 0 ; i < 16; i ++){
        for(int j = 0; j < 4; j ++){
            int row = 4 * j;
            fiveg_signal_strength[i][j] =  concatenateAndConvert(toBinary(temp[i][row]),toBinary(temp[i][row+1]),toBinary(temp[i][row+2]),toBinary(temp[i][row+3]));
...
{       
    ...
    else
    {
        if(online[count]==1)
        {
            printf("5G设备%d:10.0.%d\.2在线\n",count,count);
        }
        else
        {
            printf("5G设备%d:10.0.%d\.2离线\n",count,count);
        }
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        FiveG_detect(flag);
        Process();
        get_mesh_data(mesh_signal_strength,1);//此处的1应该为id号，这边随便写的
        Send_Topo();//mesh_signal_strength：mesh,fiveg_signal_strength:5G;5G IP:10.0.12.2,自组网不知道 IP
        sleep(sleep_time);
    }
    //服务端框架_结束
